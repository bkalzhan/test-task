...
class CreateUserViewSet(viewsets.ModelViewSet):
    ...
    def post_user(self, request):
        ...
        queryset = MainUser.objects.create_user(email=user['email'], password=user['password'],
                                                ...
                                                role=user['role'])
        queryset.save()
        logger.debug(f'debug User created ID: {user}')
        logger.info(f'info User created ID: {user}')
        logger.warning(f'warning User created ID: {user}')
        logger.error(f'error User created ID: {user}')
        logger.critical(f'critical User created ID: {user}')
